...
def delete_by_index(my_list, k):
    """Удаляет из списка элемент с индексом k, сдвинув влево все элементы, стоящие правее элемента с индексом k.
     my_list - список, из которого необходимо удалить элемент. k - индекс элемента, который необходимо удалить.
"""
...
def insert_number(my_list, k, c):
    """Вставляет в список на позицию с индексом k значение C, сдвинув все элементы, с индексом большим k, вправо."""
...
def amount_unique_numbers(my_list1, my_list2):
    """Подсчитывает сколько уникальных чисел содержится одновременно как в первом списке, так и во втором."""
    return len([x for x in (my_list1 + my_list2) if (my_list1 + my_list2).count(x) == 1])
