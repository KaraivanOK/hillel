"""Модуль содержит функции: возведения чисел из списка в квадрат, нахождения
простых чисел в заданном диапазоне и
проверки на нахождение в списке двух чисел, сумма которых равна заданному
числу."""
...
def two_sum(nums, target):  # Создаём функцию с двумя параметрами.
   ...
   булевое значение - результат поиска фукции."""
    # Задаём диапазон индекса для первого числа в сумме.
    for x in range(0, len(nums)):
        i = nums[x]  # Принимаем за первое число в сумме.
        # Принимаем за второе число через разность суммы и первого числа.
        j = target - i
        # Задаём диапазон для второго числа в сумме.
        for y in range(x + 1, len(nums)):
            # Если предполагаемое второе число находится в списке.
            if j == nums[y]:
                return True  # Выводим True.
            else:
                continue  # В противном случае начинаем следующий проход цикла.
    #  Если после всех проходов цикла второго числа в нём так и не оказалось, то
    #  выводим False.
    return False
...
def prime_number(a, b):  # Создаём функцию с двумя аргументами.
    """
       Функция нахождения простых чисел в дипазоне заданным 2мя аргументами
       чисел.
    """
    ...
    for i in range(a, b + 1):  # Пробегаем все числа от a до b включительно.
        # Ищем делители среди списка чисел не превышающих делимое.
        for j in range(2, i):
            if i % j == 0:  # Если делитель найден, число не простое.
                # Позволяет завершить выполнение внутреннего цикла и перейти к
                # следующей итерации внешнего.
                break
        else:
