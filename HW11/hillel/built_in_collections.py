...
__all__ = ['word_frequency', 'maximum_number_of_word_mentions', 'quantity_of_different_numbers', 'confluence']


def word_frequency(text):
    """Для каждого слова из заданного текста подсчитывает, сколько раз оно встречалось в этом тексте."""
...
def maximum_number_of_word_mentions(text_1, text_2, text_3):
    """Задаются три строки. Функция выводит слово, которое в этом тексте встречается чаще всего. Если таких
    ...
    return (list(reversed(words_dict.keys())))[list(reversed(words_dict.values())).index(max(dict.values(words_dict)))]


def quantity_of_different_numbers(nums):
    """Из заданного списка чисел определяет сколько в нём встречается различных чисел."""
    return len(set(nums))


def confluence(nums_1, nums_2):
    """Задаются два списка чисел. Определяем количество чисел, содержащихся как в первом списке, так и во втором."""
