...
def triangle_sequence(n):
    """Функция выводит заданное количество членов треугольной
    последовательности.
    num - текущее число для вывода. Вывод num начинается с единицы,т.к. сама
    последовательность начинается с единицы.
    k - количество уже выведенных чисел num.
    В цикле будем выводить num и увеличивать k на 1.
    Каждый раз когда количество выводы определённого числа будет равно этому
    числу, счётчик вывода обнуляется и мы переходим к следующему числу."""
    num = 1
    k = 0
    for i in range(n):
        print(num, end=' ')
...
def without_00(a, b):
    """  Функция опеределяет, сколько существует последовательностей из a нулей
     и b единиц, в которых никакие два нуля не стоят рядом. При этом задаются
     два параметра функции:
  a - количество нулей;
  b - количество единиц."""
    if a > b + 1:
        return 0
...
def reverse_it(n, i=0):
 ...
 Для этого мы каждую итерацию рекурсии отделяем от n остаток применяя n%10  и
 добывляем его в i, тем самым увеличивая i с каждой итерацией. Параллельно с
 этим мы уменьшаем значение n применяя n//10. В конечном итоге это приводит к
 тому, что n = 0, а i приобретает вид развёрнутого числа n."""
    if n == 0:
        return i
    else:
...
Функция находит сумму элементов столбцов и отсортировывает столбцы по
возрастанию этих сумм, а также отсортировывает каждый нечётный столбец по
возрастанию значений снизу вверх, а каждый чётный столбец - сверху вниз. Выводит
 матрицу до сортировки и после сортировки.
        ...
        for i in range(x):
            ...
            for y in range(x - 1):
                ...
                for j in range(0, x - y - 1):
                    # Принимаем что нумерация столбцов, при визуальном просмотре
                    # матрицы начинается с 1.
                    # В самой матрице столбцы по прежнему начинают нумерацию
                    # с 0.
                    # Для того чтобы отсортировать каждый нечётный столбец по
                    # возрастанию значений снизу вверх, а каждый чётный столбец
                    # по возрастанию значений сверху вниз, записываем
                    # противоположные сравнения в каждое из условий.
                    if i % 2 == 1:
                        if massiv2[j][i] > massiv2[j + 1][i]:
                            massiv2[j][i], massiv2[j + 1][i] = \
                                massiv2[j + 1][i], massiv2[j][i]
                    if i % 2 == 0:
                        if massiv2[j][i] < massiv2[j + 1][i]:
                            massiv2[j][i], massiv2[j + 1][i] = \
                                massiv2[j + 1][i], massiv2[j][i]
